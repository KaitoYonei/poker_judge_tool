...
RSpec.describe ApiService do
  ...
  let!(:judgeapi) { Struct.new(:target) { include ApiService } }

  describe "エラーメッセージ" do

    context '6文字以上' do
      let(:target) { judgeapi::JudgeApi.new("S12 S5 D8 C4 H1 H9")}
      it '「5つのカード指定文字を半角スペース区切りで入力してください。」という文字列が返ること' do
        expect(target.valid).to include "5つのカード指定文字を半角スペース区切りで入力してください。"
      end
    end

    context '4文字以下もしくは不正な半角スペースが存在する' do
      let(:target) { judgeapi::JudgeApi.new("C1 D3 S11")}
      it '「5つのカード指定文字を半角スペース区切りで入力してください。」という文字列が返ること' do
        expect(target.valid).to include "5つのカード指定文字を半角スペース区切りで入力してください。"
      end
    end
    ...
    context '1番目の文字が不正' do
      let(:target) { judgeapi::JudgeApi.new("G1 D3 S1 D2 C5")}
    ...
    context '2番目の文字が不正' do
      let(:target) { judgeapi::JudgeApi.new("S3 S14 S1 S7 S8")}
    ...
    context '3番目の文字が不正' do
      let(:target) { judgeapi::JudgeApi.new("C11 D2 S H10 C5")}
    ...
    context '4番目の文字が不正' do
      let(:target) { judgeapi::JudgeApi.new("H1 S2 H3 k! D1")}
    ...
    context '5番目の文字が不正' do
      let(:target) { judgeapi::JudgeApi.new("S1 H2 D12 C3 hgf")}
    ...
    context '重複したカードが存在する' do
      let(:target) { judgeapi::JudgeApi.new("S4 S11 C1 D3 S11")}
  ...
  describe "リザルトメッセージ" do
    ...
    context '同じスートで数字が連続する5枚のカードで構成されている' do
      let(:target) { judgeapi::JudgeApi.new("C5 C3 C6 C4 C7")}
      it '「ストレートフラッシュ」という文字列が返ること' do
        expect(target.judge).to eq ('ストレートフラッシュ')
      end
      let(:target) { judgeapi::JudgeApi.new("D1 D13 D11 D12 D10")}
      it '「ストレートフラッシュ」という文字列が返ること' do
        expect(target.judge).to eq ('ストレートフラッシュ')
      end
    end
    ...
    context '同じ数字のカードが4枚含まれる' do
      let(:target) { judgeapi::JudgeApi.new("H3 S3 D3 H1 C3")}
    ...
    context '同じスートのカード5枚で構成されている' do
      let(:target) { judgeapi::JudgeApi.new("H11 H12 H13 H2 H1")}
    ...
    context '数字が連続した5枚のカードによって構成されている' do
      let(:target) { judgeapi::JudgeApi.new("H1 S2 C5 C4 C3")}
      it '「ストレート」という文字列が返ること' do
        expect(target.judge).to eq ('ストレート')
      end
      let(:target) { judgeapi::JudgeApi.new("S11 D13 S12 C9 D10")}
      it '「ストレート」という文字列が返ること' do
        expect(target.judge).to eq ('ストレート')
      end
    end
    ...
    context '同じ数字の札3枚と数字の違う2枚の札から構成されている' do
      let(:target) { judgeapi::JudgeApi.new("H12 S12 C12 C4 C2")}
    ...
    context '同じ数の2枚組を2組と他のカード1枚で構成されている' do
      let(:target) { judgeapi::JudgeApi.new("H2 S6 C4 H4 D2")}
    ...
    context '同じ数字の2枚組とそれぞれ異なった数字の札3枚によって構成されている' do
      let(:target) { judgeapi::JudgeApi.new("H2 S12 C2 H4 D1")}
    ...
    context '役が1つも成⽴しない' do
      let(:target) { judgeapi::JudgeApi.new("H8 S1 C4 H3 H2")}
