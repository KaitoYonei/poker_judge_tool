...
RSpec.describe HandsController, type: :controller do
  ...
  shared_examples "不正なカードに関するエラーメッセージが返されるはず" do
  ...
  context '4文字以下もしくは不正な半角スペースが存在する場合' do
    it 'bodyに「5つのカード指定文字を半角スペース区切りで入力してください。（例: S1 H3 D9 C13 S11)」という記述があること' do
  ...
  context '1番目の文字が不正な場合' do
    ...
    it_behaves_like "不正なカードに関するエラーメッセージが返されるはず"
  end

  context '2番目の文字が不正な場合' do
    before do
